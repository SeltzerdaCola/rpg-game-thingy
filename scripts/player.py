...
class Player(p.sprite.DirtySprite):
	def __init__(self, x, y, game):
		...
		#Sprite image
		self.spritesheet = p.transform.scale(p.image.load("assets/player-sprites.png").convert_alpha(), game.settings.TileMultiply(4, 3))
		#0 = Top 1 = Bottom 2 = Left 3 = Right
		self.facing = 1
		self.currentsprite = [0, 0, *game.settings.TileArea]
		self.spritepos = [0, 0]
		self.visible = 1
		self.ui = 0
		self.layer = 2
		...
		self.image = p.Surface(game.settings.TileArea, p.SRCALPHA)
		
		#Speed, Sprint, and Stamina
		self.normal_speed = 170
		self.speed = self.normal_speed
		self.sprint_multiplyer = 1.5
